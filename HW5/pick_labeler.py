...
from utils.image import write_rgb
from utils.common import draw_grasp, get_splits
...
def main():
    ...
    for name_idx, name in enumerate(names):
        if len(labels[name]) >= n_picks:
            ...
            continue
        start_idx = len(labels[name]) + 1
        env.load_ycb_objects([name], seed=name_idx*100+start_idx)
        while start_idx < n_picks + 1:
            #seed = name_idx*100+start_idx
            env.reset_objects()
            print('Labeling {}_{}'.format(name, start_idx))
            ...
            if result:
                # save
                print("Grasp success! Saved {}/{} example".format(start_idx,n_picks))
