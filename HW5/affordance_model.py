...
class AffordanceDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        rotator = iaa.Rotate(rotate = -nn_angle)
        rot_img, rot_kps = rotator(image=rgb, keypoints=kps)
        rot_img = rot_img.astype(np.float32) / 255.0  # Normalize to 0-1
        goal_img = get_gaussian_scoremap(shape = (rgb.shape[0],rgb.shape[1]),keypoint = np.array([rot_kps[0].x,rot_kps[0].y]))
        data = {
            'input': torch.from_numpy(np.moveaxis(rot_img, -1, 0)),
            'target': torch.from_numpy(np.expand_dims(goal_img, 0)), 
        }
        assert data['input'].shape == (3, 128, 128), f"Expected input shape (3,128,128), got {data['input'].shape}"
        assert data['target'].shape == (1, 128, 128), f"Expected target shape (1,128,128), got {data['target'].shape}"
        return data
...
class AffordanceModel(nn.Module):
    ...
    def suppress_failure_grasp(self, affordance_map): 
        ...
        print("self.past_actions = "+ f"{self.past_actions}")
        update_affordance = affordance_map.clone()
        for past_coord in self.past_actions:
            suppression_map = get_gaussian_scoremap(shape = (affordance_map.shape[1],affordance_map.shape[2]),keypoint = np.array([past_coord[1],past_coord[2]]))
            update_affordance -= torch.from_numpy(suppression_map).to(update_affordance.device)
        update_affordance.clip(0.,1.)
    ...
    def predict_grasp(
        ...
        # print("rgb_obs = " + f"{rgb_obs.shape}")
        rgb_input = np.array([rotator(image = rgb_obs) for rotator in rotators])
        ...
        with torch.no_grad():
            ...
            assert input_batch.shape == (8, 3, 128, 128), f"Expected shape (8,3,128,128), got {input_batch.shape}"
