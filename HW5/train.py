...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        left_coord, right_coord = get_finger_points(center,angle)
        kps = KeypointsOnImage([
                Keypoint(x=left_coord[0], y= left_coord[1]),
                Keypoint(x=right_coord[0], y= right_coord[1]),
                ], shape=img.shape)
        seq = iaa.Sequential([self.aug_pipeline])
        rgb_aug, kps_aug = seq(image=img, keypoints=kps)
        center, angle = get_center_angle(
            np.array([kps_aug[0].x, kps_aug[0].y]),
            np.array([kps_aug[1].x, kps_aug[1].y])
        )
...
def main():
    ...
    # Then load checkpoint if it exists
