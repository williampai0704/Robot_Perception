...
class TestTransforms(unittest.TestCase):
    ...
    def test_rgbd_image_to_points(self): 
        ## final on a real RGB-D image
        im = cv2.cvtColor(cv2.imread("frame-000000.color.png"), cv2.COLOR_BGR2RGB)
        depth_image = cv2.imread('frame-000000.depth.png',-1).astype(float)
        depth_image /= 1000. # millimeters to meters
        camera_intrensics = np.array([[231.06934494, 0., 160.],
                                        [0., 231.06934494, 120.],
                                        [0., 0., 1.]])
        pts = depth_to_point_cloud(camera_intrensics, depth_image)
        write_ply('test.ply', pts, im.reshape(-1,3))
